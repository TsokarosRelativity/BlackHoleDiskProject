...
# Only include rows where the values in column 'r' are within the tolerance of rs
tolerance = 0.00001 
...
df['K_rp'] = (df.He*np.sin(df.theta)**2) / (df.psi**2 * df.r**2) #don't include second term (it is zero but also divides by zero)
df['K_tp'] = (df.Hf*np.sin(df.theta)) / (df.psi**2 * df.r) 
df['P'] = K*(df.rho**gamma)
...
ret_df.to_hdf('3D_data/horizon_data.h5', key='df', mode='w')

# rho_df = df[['x', 'y', 'z','rho']]
# rho_df.to_hdf('3D_data/horizon_data.h5', key='df', mode='w')
print(f"Run took: {time.time() - start_time} seconds 3D_data/horizon_data.h5")
