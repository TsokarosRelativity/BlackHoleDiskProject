...
parser.add_argument("folder", type=str, help="args.folder in which the Initial Data is being stored and calculated")
parser.add_argument('--v', action='store_true', help="showing details while running routine")
...
df = pd.read_csv(args.folder + file_path, delimiter=r'\s+', header=None, names=column_names)
s2 = time.time()
if args.v:
...
df["F_vp"] = np.where((~(df["b2"] < 0)) & (~(df["psi"] == 0)), np.sign(df["b_phi"])*np.abs(np.sqrt(df["b2"]))*df["r"]*(np.exp(2*df["q"]))*((df["psi"])**4), np.float64(0) )
#df['F_vp'] = df['F_vp'].replace([np.nan, np.inf, -np.inf], 0)

pivot = df.pivot_table(index='theta', columns='r', values='F_vp', aggfunc='first').astype("float64")
...
A_phi_df = pd.DataFrame(A_phi, index=pivot.index, columns=pivot.columns.astype('float64'))
#print(A_phi_df)
