...
df["J_10"] = np.cos(df.phi)*np.cos(df.theta)/df.r
df["J_11"] = np.sin(df.phi)*np.cos(df.theta)/df.r 
df["J_12"] = - np.sin(df.theta)/df.r
...
df.loc[cond1, "K_rp"] = (df.He * (np.sin(df.theta)**2))/((df.psi*df.r)**2)

df.loc[~cond1, "K_rp"] = (df.He * (np.sin(df.theta)**2))/((df.psi*df.r)**2) + (df.dbetatdr*(df.r*np.sin(df.theta)*(df.psi)**2)**2)/(2*df.alpha)

df.loc[cond1, "K_tp"] = (df.Hf*np.sin(df.theta))/(df.r*(df.psi**2))

df.loc[~cond1, "K_tp"] = (df.Hf*np.sin(df.theta))/(df.r*(df.psi**2)) + (df.dbetatdt*(df.r*np.sin(df.theta)*(df.psi)**2)**2)/(2*df.alpha)
...
horizondf["x"] = df.r * np.sin(df.theta) * np.cos(df.phi)
horizondf["y"] = df.r * np.sin(df.theta) * np.sin(df.phi)
horizondf["z"] = df.r * np.cos(df.theta)
