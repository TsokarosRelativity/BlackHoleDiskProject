...
! phi functions:
! phi1: multiquadric
! phi2: inverse multiquadric
! phi3: thin plate spline
! phi4: gaussian

program test2dinterp
  use iso_fortran_env, only: real64, int64
  use rbf2d
  implicit none

  integer, parameter :: s2 = 3, nd = 1000
  real(real64), dimension(1000, 3) :: d3d
  real(real64), dimension(80, 3) :: i3d
  real(real64) :: i, j, k, ind
  
  print *, "generating grid data ..."
  ind = 1
  do i = 1, 1000, 2
    do j = -500, 500, 2
      d3d(ind, 1) = real(i, kind=real64)
      d3d(ind, 2) = real(j, kind=real64)
      d3d(ind, 3) = real(sqrt(j*j*sqrt(i)), kind=real64)
      print *, ind, d3d(ind, 1), d3d(ind, 2), d3d(ind, 3)
      ind = ind + 1
    end do
  end do
  !subroutine rbf_interp( m, nd, xd, r0, phi, w, ni, xi, fi )
