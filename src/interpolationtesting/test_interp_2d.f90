...
!
!    Input, real ( kind = rk ) X(N), Y(N), the evalution points.
!
...
!    Input, real ( kind = rk ) X(N), Y(N), the evalution points.
!
!    Output, real ( kind = rk ) FX(N), FY(N), the first derivative values.
!
  implicit none
...
!    Input, real ( kind = rk ) X(N), Y(N), the evalution points.
!
!    Output, real ( kind = rk ) FXX(N), FXY(N), FYY(N), the second derivatives.
!
  implicit none
...
!
  ...
  f(1:n) = &
      0.75D+00 * exp ( - ( ( 9.0D+00 * x(1:n) - 2.0D+00 )**2                &
                         + ( 9.0D+00 * y(1:n) - 2.0D+00 )**2 ) / 4.0D+00 )  &
    + 0.75D+00 * exp ( - ( ( 9.0D+00 * x(1:n) + 1.0D+00 )**2 ) / 49.0D+00   &
                         - ( 9.0D+00 * y(1:n) + 1.0D+00 )      / 10.0D+00 ) &
    + 0.5D+00  * exp ( - ( ( 9.0D+00 * x(1:n) - 7.0D+00 )**2                &
                         + ( 9.0D+00 * y(1:n) - 3.0D+00 )**2 ) / 4.0D+00 )  &
    - 0.2D+00  * exp ( - (   9.0D+00 * x(1:n) - 4.0D+00 )**2                &
                         - ( 9.0D+00 * y(1:n) - 7.0D+00 )**2 )
...
!    Input, real ( kind = rk ) X(N), Y(N), the evalution points.
!
!    Output, real ( kind = rk ) FX(N), FY(N), the derivative values.
!
  implicit none
  ...
  t1(1:n) = exp ( - ( ( 9.0D+00 * x(1:n) - 2.0D+00 )**2 &
                    + ( 9.0D+00 * y(1:n) - 2.0D+00 )**2 ) / 4.0D+00 )
  t2(1:n) = exp ( - ( ( 9.0D+00 * x(1:n) + 1.0D+00 )**2 ) / 49.0D+00 &
                    - ( 9.0D+00 * y(1:n) + 1.0D+00 )      / 10.0D+00 )
  t3(1:n) = exp ( - ( ( 9.0D+00 * x(1:n) - 7.0D+00 )**2 &
                    + ( 9.0D+00 * y(1:n) - 3.0D+00 )**2 ) / 4.0D+00 )
  t4(1:n) = exp ( -   ( 9.0D+00 * x(1:n) - 4.0D+00 )**2 &
                    - ( 9.0D+00 * y(1:n) - 7.0D+00 )**2 )

  fx(1:n) = &
    - 3.375D+00               * ( 9.0D+00 * x(1:n) - 2.0D+00 ) * t1 &
    - ( 27.0D+00 / 98.0D+00 ) * ( 9.0D+00 * x(1:n) + 1.0D+00 ) * t2 &
    - 2.25D+00                * ( 9.0D+00 * x(1:n) - 7.0D+00 ) * t3 &
    + 3.6D+00                 * ( 9.0D+00 * x(1:n) - 4.0D+00 ) * t4

  fy(1:n) = &
    - 3.375D+00 * ( 9.0D+00 * y(1:n) - 2.0D+00 ) * t1 &
    - 0.675D+00                                  * t2 &
    - 2.25D+00  * ( 9.0D+00 * y(1:n) - 3.0D+00 ) * t3 &
    + 3.6D+00   * ( 9.0D+00 * y(1:n) - 7.0D+00 ) * t4
...
!    Input, real ( kind = rk ) X(N), Y(N), the evalution points.
!
!    Output, real ( kind = rk ) FXX(N), FXY(N), FYY(N), second derivatives.
!
  implicit none
  ...
  fxx(1:n) = &
      15.1875D+00 * ( ( 9.0D+00 * x(1:n) - 2.0D+00 )**2 - 2.0D+00 )  * t1(1:n) &
    + 60.75D+00   * ( ( 9.0D+00 * x(1:n) + 1.0D+00 )**2 - 24.5D+00 ) * t2(1:n) &
    + 10.125D+00  * ( ( 9.0D+00 * x(1:n) - 7.0D+00 )**2 - 2.0D+00 )  * t3(1:n) &
    - 64.8D+00    * ( ( 9.0D+00 * x(1:n) - 4.0D+00 )**2 - 0.5D+00 )  * t4(1:n)

  fxy(1:n) = &
      15.1875D+00 * ( 9.0D+00 * x(1:n) - 2.0D+00 ) &
    * ( 9.0D+00 * y(1:n) - 2.0D+00 ) * t1(1:n) &
    + ( 243.0D+00 / 980.0D+00 ) * ( 9.0D+00 * x(1:n) + 1.0D+00 ) * t2(1:n) &
    + 10.125D+00 * ( 9.0D+00 * x(1:n) - 7.0D+00 ) &
    * ( 9.0D+00 * y(1:n) - 3.0D+00 ) * t3(1:n) &
    - 64.8D+00 * ( 9.0D+00 * x(1:n) - 4.0D+00 ) &
    * ( 9.0D+00 * y(1:n) - 7.0D+00 ) * t4(1:n)

  fyy(1:n) = &
      15.1875D+00 * ( ( 9.0D+00 * y(1:n) - 2.0D+00 )**2 - 2.0D+00 ) * t1(1:n) &
    + 0.6075D+00  *                                                   t2(1:n) &
    + 10.125D+00  * ( ( 9.0D+00 * y(1:n) - 3.0D+00 )**2 - 2.0D+00 ) * t3(1:n) &
    - 64.8D+00    * ( ( 9.0D+00 * y(1:n) - 7.0D+00 )**2 - 0.5D+00 ) * t4(1:n)
...
!
  ...
  f(1:n) = ( tanh ( 9.0D+00 * ( y(1:n) - x(1:n) ) ) + 1.0D+00 ) / 9.0D+00
...
!
  ...
  t1(1:n) = 18.0D+00 * ( y(1:n) - x(1:n) )
  fx(1:n) = - 4.0D+00 / ( exp ( t1(1:n) ) + 2.0D+00 + exp ( - t1(1:n) ) )
  fy(1:n) = - fx(1:n)
...
!
  ...
  fxx(1:n) = 18.0D+00 * tanh ( 0.5D+00 * t1(1:n) ) &
    * ( tanh ( 9.0D+00 * ( y(1:n) - x(1:n) ) ) + 1.0D+00 ) / 9.0D+00
  fxy(1:n) = - fxx(1:n)
  fyy(1:n) = fxx(1:n)
...
!
  ...
  f(1:n) = ( 1.25D+00 + cos ( 5.4D+00 * y(1:n) ) ) &
    / ( 6.0D+00 + 6.0D+00 * ( 3.0D+00 * x(1:n) - 1.0D+00 )**2 )
...
!
  ...
  t1(1:n) = 5.4D+00 * y(1:n)
  t2(1:n) = 1.0D+00 + ( 3.0D+00 * x(1:n) - 1.0D+00 )**2
  fx(1:n) = - ( 3.0D+00 * x(1:n) - 1.0D+00 ) &
    * ( 1.25D+00 + cos ( t1(1:n) ) ) / ( t2(1:n) * t2(1:n) )
  fy(1:n) = - 0.9D+00 * sin ( t1(1:n) ) / t2(1:n)
...
!
  ...
  fxx(1:n) = 3.0D+00 * ( 1.25D+00 + cos ( t1(1:n) ) ) &
    * ( 3.0D+00 * t2(1:n) - 4.0D+00 ) / ( t2(1:n)**3 )
  fxy(1:n) = 5.4D+00 * ( 3.0D+00 * x(1:n) - 1.0D+00 ) * sin ( t1(1:n) ) &
    / ( t2(1:n) * t2(1:n) )
  fyy(1:n) = - 4.86D+00 * cos ( t1(1:n) ) / t2(1:n)
...
!
  ...
  f(1:n) = exp ( - 5.0625D+00 * ( ( x(1:n) - 0.5D+00 )**2 &
                                + ( y(1:n) - 0.5D+00 )**2 ) ) / 3.0D+00
...
!
  ...
  t1(1:n) = x(1:n) - 0.5D+00
  t2(1:n) = y(1:n) - 0.5D+00
  t3(1:n) = - 3.375D+00 &
    * exp ( - 5.0625D+00 * ( t1(1:n) * t1(1:n) + t2(1:n) * t2(1:n) ) )
...
!
  ...
  fxx(1:n) = ( 1.0D+00 - 10.125D+00 * t1(1:n) * t1(1:n) ) * t3(1:n)
  fxy(1:n) = - 10.125D+00 * t1(1:n) * t2(1:n) * t3(1:n)
  fyy(1:n) = ( 1.0D+00 - 10.125D+00 * t2(1:n) * t2(1:n) ) * t3(1:n)
...
!
  ...
  f(1:n) = exp ( - 20.25D+00 &
    * ( ( x(1:n) - 0.5D+00 )**2 + ( y(1:n) - 0.5D+00 )**2 ) ) / 3.0D+00
...
!
  ...
  t2(1:n) = y(1:n) - 0.5D+00
  t3(1:n) = - 13.5D+00 &
    * exp ( - 20.25D+00 * ( t1(1:n) * t1(1:n) + t2(1:n) * t2(1:n) ) )
  fx(1:n) = t1(1:n) * t3(1:n)
  fy(1:n) = t2(1:n) * t3(1:n)
...
!
  ...
  fxx(1:n) = ( 1.0D+00 - 40.5D+00 * t1(1:n) * t1(1:n) ) * t3(1:n)
  fxy(1:n) = - 40.5D+00 * t1(1:n) * t2(1:n) * t3(1:n)
  fyy(1:n) = ( 1.0D+00 - 40.5D+00 * t2(1:n) * t2(1:n) ) * t3(1:n)
...
!
  ...
  t4(1:n) = 64.0D+00 - 81.0D+00 &
    * ( ( x(1:n) - 0.5D+00 )**2 + ( y(1:n) - 0.5D+00 )**2 )

  do i = 1, n

    if ( 0.0D+00 <= t4(i) ) then
      f(i) = sqrt ( t4(i) ) / 9.0D+00 - 0.5D+00
    else
      f(i) = 0.0D+00
    end if
...
!
  ...
  f(1:n) = 2.0D+00 * cos ( 10.0D+00 * x(1:n) ) * sin ( 10.0D+00 * y(1:n) ) &
    + sin ( 10.0D+00 * x(1:n) * y(1:n) )
...
!
  ...
  t1(1:n) = 10.0D+00 * x(1:n)
  t2(1:n) = 10.0D+00 * y(1:n)
  t3(1:n) = 10.0D+00 * cos ( 10.0D+00 * x(1:n) * y(1:n) )
  fx(1:n) = - 20.0D+00 * sin ( t1(1:n) ) * sin ( t2(1:n) ) + t3(1:n) * y(1:n)
  fy(1:n) =   20.0D+00 * cos ( t1(1:n) ) * cos ( t2(1:n) ) + t3(1:n) * x(1:n)
...
!
  ...
  t2(1:n) = 10.0D+00 * y(1:n)
  t3(1:n) = 10.0D+00 * cos ( 10.0D+00 * x(1:n) * y(1:n) )
  t4(1:n) = 100.0D+00 * sin ( 10.0D+00 * x(1:n) * y(1:n) )

  fxx(1:n) = - 200.0D+00 * cos ( t1(1:n) ) * sin ( t2(1:n) ) &
    - t4(1:n) * y(1:n) * y(1:n)
  fxy(1:n) = - 200.0D+00 * sin ( t1(1:n) ) * cos ( t2(1:n) ) &
    + t3(1:n) - t4(1:n) * x(1:n) * y(1:n)
  fyy(1:n) = - 200.0D+00 * cos ( t1(1:n) ) * sin ( t2(1:n) ) &
    - t4(1:n) * x(1:n) * x(1:n)
...
!
  ...
  t1(1:n) = 5.0D+00 - 10.0D+00 * x(1:n)
  t2(1:n) = 5.0D+00 - 10.0D+00 * y(1:n)
  t3(1:n) = exp ( - 0.5D+00 * t1(1:n) * t1(1:n) )
  t4(1:n) = exp ( - 0.5D+00 * t2(1:n) * t2(1:n) )
  f(1:n) = t3(1:n) + 0.75D+00 * t4(1:n) * ( 1.0D+00 + t3(1:n) )
...
!
  ...
  fx(1:n) = t1(1:n) * t3(1:n) * ( 10.0D+00 + 7.5D+00 * t4(1:n) )
  fy(1:n) = t2(1:n) * t4(1:n) * ( 7.5D+00 + 7.5D+00 * t3(1:n) )
...
!
  ...
  fxx(1:n) = t3(1:n) * ( t1(1:n) * t1(1:n) - 1.0D+00 ) &
    * ( 100.0D+00 + 75.0D+00 * t4(1:n) )
  fxy(1:n) = 75.0D+00 * t1(1:n) * t2(1:n) * t3(1:n) * t4(1:n)
  fyy(1:n) = t4(1:n) * ( t2(1:n) * t2(1:n) - 1.0D+00 ) &
    * ( 75.0D+00 + 75.0D+00 * t3(1:n) )
...
!
  ...
  t1(1:n) = exp ( ( 10.0D+00 - 20.0D+00 * x(1:n) ) / 3.0D+00 )
  t2(1:n) = exp ( ( 10.0D+00 - 20.0D+00 * y(1:n) ) / 3.0D+00 )
  t3(1:n) = 1.0D+00 / ( 1.0D+00 + t1(1:n) )
  t4(1:n) = 1.0D+00 / ( 1.0D+00 + t2(1:n) )
  f(1:n) = ( ( 20.0D+00 / 3.0D+00 )**3 * t1(1:n) * t2(1:n) )**2 &
    * ( t3(1:n) * t4(1:n) )**5 &
    * ( t1(1:n) - 2.0D+00 * t3(1:n) ) * ( t2(1:n) - 2.0D+00 * t4(1:n) )
...
!
  ...
  fx(1:n) = ( ( 20.0D+00 / 3.0D+00 ) * t1(1:n) )**2 &
    * ( ( 20.0D+00 / 3.0D+00 ) * t3(1:n) )**5 &
    * ( 2.0D+00 * t1(1:n) - 3.0D+00 * t3(1:n) - 5.0D+00 &
    + 12.0D+00 * t3(1:n) * t3(1:n) ) &
    * t2(1:n) * t2(1:n) * t4(1:n)**5 &
    * ( t2(1:n) - 2.0D+00 * t4(1:n) )

  fy(1:n) = ( ( 20.0D+00 / 3.0D+00 ) * t1(1:n) )**2 &
    * ( ( 20.0D+00 / 3.0D+00 ) * t3(1:n) )**5 &
    * ( 2.0D+00 * t2(1:n) - 3.0D+00 * t4(1:n) - 5.0D+00 &
    + 12.0D+00 * t4(1:n) * t4(1:n) ) &
    * t2(1:n) * t2(1:n) * t4(1:n)**5 &
    * ( t1(1:n) - 2.0D+00 * t3(1:n) )
...
!
  ...
  fxx(1:n) = t5 * t6(1:n) * ( t2(1:n) - 2.0D+00 * t4(1:n) ) &
    * ( ( ( - 84.0D+00 * t3(1:n) + 78.0D+00 ) * t3(1:n) + 23.0D+00 ) * t3(1:n) &
    + 4.0D+00 * t1(1:n) - 25.0D+00 )

  fxy(1:n) = t5 * t6(1:n) &
    * ( ( 12.0D+00 * t4(1:n) - 3.0D+00 ) * t4(1:n) &
    + 2.0D+00 * t2(1:n) - 5.0D+00 ) &
    * ( ( 12.0D+00 * t3(1:n) - 3.0D+00 ) &
    * t3(1:n) + 2.0D+00 * t1(1:n) - 5.0D+00 )

  fyy(1:n) = t5 * t6(1:n) * ( t1(1:n) - 2.0D+00 * t3(1:n) ) &
    * ( ( ( - 84.0D+00 * t4(1:n) + 78.0D+00 ) * t4(1:n) + 23.0D+00 ) * t4(1:n) &
    + 4.0D+00 * t2(1:n) - 25.0D+00 )
...
!
  ...
  t1(1:n) = sqrt ( ( 80.0D+00 * x(1:n) - 40.0D+00 )**2 &
    + ( 90.0D+00 * y(1:n) - 45.0D+00 )**2 )
  t2(1:n) = exp ( - 0.04D+00 * t1(1:n) )
  t3(1:n) = cos ( 0.15D+00 * t1(1:n) )
...
!
  ...
  do i = 1, n
    ...
    if ( t1(i) == 0.0D+00 ) then
      fx(i) = 0.0D+00
      fy(i) = 0.0D+00
    else
      t2 = exp ( - 0.04D+00 * t1(i) )
      t3 = cos ( 0.15D+00 * t1(i) )
      t4 = sin ( 0.15D+00 * t1(i) )
      fx(i) = - t2 * ( 12.0D+00 * t4 + 3.2D+00 * t3 ) &
        * ( 80.0D+00 * x(i) - 40.0D+00 ) / t1(i)
      fy(i) = - t2 * ( 13.5D+00 * t4 + 3.6D+00 * t3 ) &
        * ( 90.0D+00 * y(i) - 45.0D+00 ) / t1(i)
    end if
...
!
  ...
  do i = 1, n
    ...
    if ( t1(i) == 0.0D+00 ) then
      ...
      fxx(i) = t5 * ( t1(i) * ( 76.8D+00 * t4 - 133.76D+00 * t3 ) &
        * ( 80.0D+00 * x(i) - 40.0D+00 )**2 &
        - ( 960.0D+00 * t4 + 256.0D+00 * t3 ) &
        * ( 90.0D+00 * y(i) - 45.0D+00 )**2 )

      fxy(i) = t5 * ( t1(i) * ( 86.4D+00 * t4 - 150.48D+00 * t3 ) &
        + 1080.0D+00 * t4 + 288.0D+00 * t3 ) * ( 80.0D+00 * x(i) - 40.0D+00 ) &
        * ( 90.0D+00 * y(i) - 45.0D+00 )

      fyy(i) = t5 * ( t1(i) * ( 97.2D+00 * t4 - 169.29D+00 * t3 ) &
        * ( 90.0D+00 * y(i) - 45.0D+00 )**2 &
...
!
!    Output, real ( kind = rk ) GX(GN), GY(GN), the grid coordinates.
!
  implicit none
